...
**PR 31 — The CFG first.** Loops were named `file:line ($L__BB0_2)`
with nothing in the report saying what `$L__BB0_2` was. Each kernel
now opens with its block table (`blocks` in JSON, a column table in
text): every basic block in program order with its label (or
